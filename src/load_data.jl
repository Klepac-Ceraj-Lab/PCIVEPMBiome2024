...
function load_pcimbiome_data(paper_sample_set; gene_min_prevalence = 0.01) # Take note of prevalence filter used for the calculations!
    #1. Core metadata
    ...
    unstratified_unirefs_filtered = let filt = get(unstratified_unirefs, :MaternalEntropy) .>= 0.0
        keepuni = vec(prevalence(unstratified_unirefs[:, filt]) .> gene_min_prevalence)
    ...
    stratified_unirefs_filtered = let filt = get(stratified_unirefs, :MaternalEntropy) .>= 0.0
        keepuni = vec(prevalence(stratified_unirefs[:, filt]) .> gene_min_prevalence)
    ...
    return mdata_cols, khula_pci_mbiome_data, unstratified_unirefs, stratified_unirefs, unstratified_unirefs_filtered, stratified_unirefs_filtered
end